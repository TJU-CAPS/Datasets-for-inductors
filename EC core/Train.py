...
def get_dataset(file_path=None):
    """加载CSV数据并进行预处理（单位转换+标准化）

    Parameters
    ----------
    file_path : str, optional
        CSV文件路径，默认使用 Datasets/Test_5000.csv
        用户可替换为自己的训练数据路径
    """
    if file_path is None:
        file_path = os.path.join(os.path.dirname(__file__), "Datasets", "Test_5000.csv")
...
def main():
    ...
    DECAY_EPOCH = 50
    DECAY_RATIO = 0.5
    LR_INI = 0.00627  # 建议Sigmoid时略微降低初始学习率
    ...
    # 加载并测试
    checkpoint = torch.load('best_model_1_0.pth', weights_only=False)
